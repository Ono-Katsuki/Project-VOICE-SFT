...
OUT_TUNED_JSONL_DEFAULT = (
    ""
    "synth_v11_tone8_tuned__strongtone.jsonl"
)
...
def load_processed_pairs_from_tuned(path: Path) -> Set[Tuple[str, str]]:
...
def main() -> None:
    ...
    if args.resume_skip_existing:
        processed_pairs = load_processed_pairs_from_tuned(out_tuned_path)
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            with lock:
                for r in recs:
                    f_tuned.write(json.dumps(r, ensure_ascii=False) + "\n")
                    gemini_line = {
                        "contents": [
