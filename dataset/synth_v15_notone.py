...
def load_processed_seeds_from_tuned(path: Path) -> Set[str]:
...
def main() -> None:
    ...
    if args.resume_skip_existing:
        processed = load_processed_seeds_from_tuned(out_tuned_path)
        ...
        for fut in as_completed(futures):
            ...
            with lock:
                ...
                if (not args.only_ok) or (rec["status"] == "ok" and rec["format_ok"]):
                    f_tuned.write(json.dumps(rec, ensure_ascii=False) + "\n")
