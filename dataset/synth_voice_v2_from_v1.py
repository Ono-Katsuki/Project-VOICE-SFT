...
- 機能:
    ...
    - 途中再開:
        既存の Tuned 出力の (seed_id, context_id) を見てスキップ
...
- 注意:
    - モデル呼び出しに失敗した (seed_id, context_id) の組は
      Tuned / Gemini どちらの出力ファイルにも一切書き出さない。
"""
...
PROJECT_ID_DEFAULT = "your-gcp-project-id"
...
TUNED_MODEL_ENDPOINT_DEFAULT = (
    "projects/YOUR_PROJECT_NUMBER/locations/us-central1/endpoints/YOUR_ENDPOINT_ID"
)

# v1 の Tuned 形式 JSONL (input_text / output_text)
V1_TUNED_JSONL_DEFAULT = (
    ""
    "gemini_sft_yomi2surface_train_mixed_with_context_10k.jsonl"
)

# 出力ファイル
OUT_TUNED_JSONL_DEFAULT = (
    ""
    "voice_v2_synth_from_v1_tuned.jsonl"
)
...
def load_processed_pairs(path: Path) -> Set[Tuple[int, str]]:
    """
    既存の Tuned 出力ファイルから (seed_id, context_id) のペア集合を復元。
    ...
    print(
        f"[DEBUG] existing Tuned lines={lines}, "
...
def main() -> None:
    ...
    ap.add_argument("--v1-tuned-jsonl", default=V1_TUNED_JSONL_DEFAULT)
    ap.add_argument("--out-tuned", default=OUT_TUNED_JSONL_DEFAULT)
    ...
    v1_path = Path(args.v1_tuned_jsonl)
    out_tuned_path = Path(args.out_tuned)
    out_gemini_path = Path(args.out_gemini)
    max_workers = max(1, min(args.max_workers, 50))
    retries = args.retries

    if not v1_path.exists():
        print(f"[ERROR] v1 Tuned JSONL not found: {v1_path}", file=sys.stderr)
        sys.exit(1)

    out_tuned_path.parent.mkdir(parents=True, exist_ok=True)
    out_gemini_path.parent.mkdir(parents=True, exist_ok=True)

    # 既存 Tuned 出力から (seed_id, context_id) セットを復元して、途中再開を実現
    processed_pairs = load_processed_pairs(out_tuned_path)
    ...
    # 出力ファイルは追記モード
    f_tuned = out_tuned_path.open("a", encoding="utf-8")
    ...
    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        ...
        for fut in as_completed(futures):
            ...
            with lock:
                f_tuned.write(json.dumps(tuned_line, ensure_ascii=False) + "\n")
                f_tuned.flush()
